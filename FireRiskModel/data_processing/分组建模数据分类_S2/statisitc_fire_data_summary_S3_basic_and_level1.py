...
程序功能: 
    ...
    4. 统计每个分类的fire_num非零值数量(所有年份之和)
    5. 统计每个分类的fire_area非零值数量(所有年份之和)
    6. 将统计结果输出到Excel文件
...
输出文件路径: 
    基础分类统计: I:/Processing data/渔网分类建模/火灾分类数据汇总/分类建模火灾数据汇总统计_Basic.xlsx
    Level-1分类统计: I:/Processing data/渔网分类建模/火灾分类数据汇总/分类建模火灾数据汇总统计_level1.xlsx
"""
...
# 输入文件路径
input_dir = r'I:\Processing data\渔网分类建模\火灾分类数据汇总'

# 输出文件路径
output_dir = input_dir
output_file_basic = os.path.join(output_dir, '分类建模火灾数据汇总统计_Basic.xlsx')
output_file_level1 = os.path.join(output_dir, '分类建模火灾数据汇总统计_level1.xlsx')
...
def process_all_years_aggregated(mapping_dict):
    ...
    for year in years:
        ...
        for _, row in df.iterrows():
            ...
            fire_freq = row['fire_num非零值数量']
            fire_norm_area = row['fire_area非零值数量']
            ...
            original_stats[new_classification_id]['fire_area_total'] += fire_norm_area
    ...
    for classification_id, stats in original_stats.items():
        stats_list.append({
            ...
            'fire_num非零值数量': stats['fire_num_total'],
            'fire_area非零值数量': stats['fire_area_total']
        })
