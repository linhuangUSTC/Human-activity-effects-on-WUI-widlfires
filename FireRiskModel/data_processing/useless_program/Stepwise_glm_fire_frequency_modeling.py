...
输入路径:
         - 火灾面积数据: I:/Processing data/渔网分类建模/分类建模数据/Fire_frequency
        - 数据格式: CSV文件,包含All_fire_frequency等必要列
...
def process_csv_file(csv_file, output_dir, failure_count):
    """
    Process a single CSV file, build GLM model and generate report and scatter plots
    
    Parameters:
    csv_file: str, CSV file path
    output_dir: str, Output directory
    failure_count: list, List used to record failure count (modified by reference)
    
    Returns:
    tuple: (category_id, selected_vars) - Category ID and list of final selected variables; returns None if failed
    """
    try:
        # Read data
        ...
        required_cols = ['All_fire_frequency', 'Roadsd', 'PopD', 'GDP', 'HFI', 'All_WUI_area']
        ...
        axes[0].set_xlabel('True Values (All_fire_frequency)')
        ...
        for i, var in enumerate(selected_vars):
            if i < max_vars_to_show:  # Show maximum 5 independent variable scatter plots
                ...
                axes[i+1].set_ylabel('All_fire_frequency')
                axes[i+1].set_title(f'{var} (selected) vs All_fire_frequency')
                axes[i+1].grid(True)
...
def main():
    ...
    input_dir = r"I:\Processing data\渔网分类建模\分类建模数据\Fire_frequency"  # Fire area data directory
