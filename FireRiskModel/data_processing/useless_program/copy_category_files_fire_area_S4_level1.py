# -*- coding: utf-8 -*-
r"""
程序名称：分类数据文件重组工具（Level 1分类版） (copy_category_files_S4_level1.py)
...
输入路径：
    - Excel文件:I:\Processing data\渔网分类建模\火灾分类数据汇总\分类建模火灾数据汇总统计_level1.xlsx
...
输出路径：
    - 目标目录:I:\Processing data\渔网分类建模\Fire_area_with_WUI_area_offset_without_0_分类建模数据_Level_1
...
# Level 1分类缩写到数字ID的映射字典
# 来自 statisitc_fire_data_summary_S3_basic_and_level1.py
c_to_lc = {
    "RCP": [10, 11, 12],
    "ICP": [20],
    "EBF": [51, 52],
...
def process_category_data(excel_file, sheet_name, output_suffix, filter_column, filter_value=0):
    ...
    source_dir_pattern = "I:\Processing data\渔网分类建模\{}"
    target_dir = "I:\Processing data\渔网分类建模\Fire_area_with_WUI_area_offset_without_0_分类建模数据_Level_1"  # 修改为新的输出目录
    ...
    try:
        ...
        for category_id in category_ids:
            ...
            if restructured_data:
                ...
                # 根据传入的过滤条件删除数据（删除值为0或空值的行）
                df_filtered = df_restructured[(df_restructured[filter_column] != filter_value) & pd.notna(df_restructured[filter_column])]
...
def main():
    # 配置参数 - 使用Level 1分类版Excel文件
    excel_file = "I:\Processing data\渔网分类建模\火灾分类数据汇总\分类建模火灾数据汇总统计_level1.xlsx"
    
    print("=== 开始处理火灾面积数据 ===")
    print("Sheet: Fire_area_over100")
    print("输出后缀: fire_area")
    print("过滤条件: All_fire_area != 0")
    # 处理火灾面积数据
