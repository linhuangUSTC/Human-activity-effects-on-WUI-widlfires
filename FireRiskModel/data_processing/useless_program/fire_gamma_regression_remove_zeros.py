...
"""
功能：
    使用简单Gamma回归模型分析5个独立变量(Roadsd, GDP, PopD, HFI, All_WUI_area)对火灾相关目标变量的解释力
    - 一次性将所有独立变量纳入模型
    - 针对每个因变量单独剔除零值后建模
输入文件：
    - 数据文件:I:/Processing data/Artificial influencing factors.csv
输出文件：
    - 可视化图表:B:/WUI/Picture and statistic results/simple_gamma_regression_results_remove_zeros_*.jpg
    - 分析报告:B:/WUI/Picture and statistic results/simple_gamma_analysis_summary_remove_zeros.txt
目标变量：
...
def data_cleaning(df, dependent_vars, independent_vars):
    ...
    # 3. 确保因变量为非负数（支持0值，但后续会针对每个因变量单独剔除0值）
...
def build_gamma_model(df_clean, dependent_var, independent_vars):
    """
    构建简单Gamma回归模型
    """
    print("\n" + "="*60)
    print(f"简单Gamma回归模型构建 - {dependent_var}")
    print("="*60)
    ...
    null_model = sm.GLM(y, np.ones((len(y), 1)), family=family).fit()
...
def plot_model_results(model, dependent_var, output_dir):
    ...
    colors = ['#4169E1'] * len(coefs)  # 统一使用蓝色
    ...
    plt.title(f'Simple Gamma Regression Coefficients - {dependent_var} (Zero Values Removed)', fontsize=18, fontweight='bold')
    ...
    # 保存图表
    filename = f"simple_gamma_regression_coefficients_remove_zeros_{dependent_var.replace(' ', '_').lower()}.jpg"
...
def save_analysis_summary(models, proportional_deviance, output_dir):
    ...
    保存分析摘要到文本文件
    """
    filename = "simple_gamma_analysis_summary_remove_zeros.txt"
    filepath = os.path.join(output_dir, filename)
    
    with open(filepath, 'w', encoding='utf-8') as f:
        f.write("="*60 + "\n")
        f.write("简单Gamma回归分析摘要（已剔除零值）\n")
        f.write("="*60 + "\n\n")
...
def main():
    ...
    # 为每个因变量构建简单Gamma回归模型（针对每个因变量单独剔除零值）
    models = {}
    proportional_deviance = {}
    
    for dep_var in dependent_vars:
        # 创建数据副本，避免影响其他因变量的处理
        df_dep = df_clean.copy()
        
        # 针对当前因变量单独剔除零值
        original_count = len(df_dep)
        df_dep = df_dep[df_dep[dep_var] > 0]
        removed_count = original_count - len(df_dep)
        print(f"\n" + "="*60)
        print(f"针对 {dep_var} 剔除零值")
        print("="*60)
        print(f"原始样本数: {original_count}")
        print(f"剔除零值样本数: {removed_count}")
        print(f"剩余样本数: {len(df_dep)}")
        print(f"零值比例: {removed_count/original_count*100:.2f}%")
        
        # 构建模型
        model, prop_dev = build_gamma_model(df_dep, dep_var, independent_vars)
