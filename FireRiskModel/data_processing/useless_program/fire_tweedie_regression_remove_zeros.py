...
功能：
    ...
    - 一次性将所有独立变量纳入模型
    - 针对每个因变量单独剔除零值后建模（分开剔除，不是联合剔除）
输入文件：
    - 数据文件:I:/Processing data/Artificial influencing factors.csv
输出文件：
    - 可视化图表:B:/WUI/Picture and statistic results/tweedie_regression_results_remove_zeros_*.jpg
    - 分析报告:B:/WUI/Picture and statistic results/tweedie_analysis_summary_remove_zeros.txt
目标变量：
...
def data_cleaning(df, dependent_vars, independent_vars):
    ...
    # 3. 确保因变量为非负数（Tweedie分布支持0值，但后续会针对每个因变量单独剔除0值）
...
def plot_model_results(model, dependent_var, output_dir):
    ...
    plt.title(f'Tweedie Regression Coefficients - {dependent_var} (Zero Values Removed)', fontsize=18, fontweight='bold')
    ...
    # 保存图表
    filename = f"tweedie_regression_coefficients_remove_zeros_{dependent_var.replace(' ', '_').lower()}.jpg"
...
def save_analysis_summary(models, proportional_deviance, output_dir):
    ...
    保存分析摘要到文本文件
    """
    filename = "tweedie_analysis_summary_remove_zeros.txt"
    filepath = os.path.join(output_dir, filename)
    
    with open(filepath, 'w', encoding='utf-8') as f:
        f.write("="*60 + "\n")
        f.write("TWEEDIE回归分析摘要（已剔除零值）\n")
        f.write("="*60 + "\n\n")
...
def main():
    ...
    # 为每个因变量构建Tweedie回归模型（针对每个因变量单独剔除零值）
    models = {}
    ...
    for dep_var in dependent_vars:
        ...
        # 构建模型
        model, prop_dev = build_tweedie_model(df_dep, dep_var, independent_vars)
