...
# 步骤1：定义输入输出路径
# 使用已投影的人口栅格数据
population_raster_dir = r"I:\Processing data\Population_equal_area_projection.gdb"
...
for wui_year in sorted(year_mapping.keys()):
    ...
    for pop_year in sorted(pop_years):
        ...
        # 执行分区统计
        arcpy.sa.ZonalStatisticsAsTable(fishnet_for_analysis, zone_field, clipped_pop_raster, temp_zonal_table, "DATA", "SUM")
