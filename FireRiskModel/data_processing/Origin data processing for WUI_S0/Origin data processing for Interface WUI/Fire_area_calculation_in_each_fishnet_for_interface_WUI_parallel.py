# -*- coding: utf-8 -*-
# --------------------------------------------------------------------------------
# 程序名称：WUI过火面积渔网统计工具（矢量核心版-并行计算）
# 功能说明：
# 1. 基于已有等面积渔网，计算WUI内各年份过火面积
# 2. 核心逻辑：WUI与过火面积相交→拆分跨栅格斑块→按GRID_ID统计求和→字段关联
# 3. 多时段并行处理，新增“过火面积年份”字段（单位：km²）
# 4. 使用多进程并行处理4个WUI年份，提高处理效率
# --------------------------------------------------------------------------------
...
# 定义路径与时间分组
# ------------------------
output_gdb = r"I:\Processing data\WUI_Fire_Risk.gdb"  # 统一存储GDB（含等面积WUI和渔网）
...
def process_wui_year(wui_year):
    """处理单个WUI年份的过火面积统计"""
    # 环境设置（在每个进程中单独设置）
    arcpy.env.workspace = output_gdb
    arcpy.env.overwriteOutput = True
    arcpy.env.parallelProcessingFactor = "100%"  # ArcPy内部并行处理设置
    ...
    wui_equal_area = os.path.join(output_gdb, "Time_series_{0}_Interface_WUI_equal_area".format(wui_year))  # 等面积WUI
    target_grid = os.path.join(output_gdb, "Grid_Clip_{0}_interface_WUI".format(wui_year))  # 已为等面积的渔网

    # 2. 循环处理每个过火年份
    for fire_year in fire_years:
        print("\n--- 处理 {0} 年过火面积 ---".format(fire_year))
        field_name = "Fire_area_{0}".format(fire_year)  # 新增字段名（英文格式）
        ...
        stats_table = os.path.join(output_gdb, "FireArea_Stats_{0}_{1}".format(wui_year, fire_year))  # 面积统计临时表
        ...
        # 统计split_fire中每个GRID_ID的SHAPE_Area总和
...
if __name__ == "__main__":
    # 创建进程池，使用4个进程（对应4个WUI年份）
    print("===== 开始并行计算各WUI年份的过火面积 =====")
    print("处理的WUI年份：{0}".format(list(wui_year_mapping.keys())))
    print("输出结果位置：{0}".format(output_gdb))
    
    # 启动并行计算
    pool = multiprocessing.Pool(processes=4)
    try:
        results = pool.map(process_wui_year, sorted(wui_year_mapping.keys()))
    finally:
        pool.close()
        pool.join()
    ...
    print("成功处理的WUI年份：{0}".format(results))