# -*- coding: utf-8 -*-
# -------------------------------------------------------------------------------------#
# 程序名称：WUI区域多时段火点统计分析工具（多字段版 - 并行计算）
# 功能描述：
...
# 4. 使用并行计算同时处理4个WUI年份，提高效率
#
# 输入数据：
#   - 野火起火点数据：GFA_v20240409_ignitions_年份.shp（2003-2022年）
#   - WUI范围数据：Time_series_2005/2010/2015/2020_Intermix_WUI.shp
#   - 有效栅格数据：WUI_Fire_Risk.gdb中的Grid_Clip_年份_intermix_WUI（EPSG:54034）
#
...
# 定义路径与时间分组
# ------------------------
output_dir = r"E:\data-hl\Processing data"  # 输出文件夹
...
def process_wui_year(wui_year):
    ...
    # 1. 定义数据路径
    wui_shp = os.path.join(output_gdb, "Time_series_{0}_Intermix_WUI_equal_area".format(wui_year))  # WUI原始数据
    target_grid = os.path.join(output_gdb, "Grid_Clip_{0}_intermix_WUI".format(wui_year))  # 目标渔网栅格（GDB内）
    ...
    for fire_year in fire_years:
        ...
        # 中间结果路径（GDB内要素类）
        fire_in_wui = os.path.join(output_gdb, "Fire_ignitions_in_{0}_Intermix_WUI_{1}".format(wui_year, fire_year))
