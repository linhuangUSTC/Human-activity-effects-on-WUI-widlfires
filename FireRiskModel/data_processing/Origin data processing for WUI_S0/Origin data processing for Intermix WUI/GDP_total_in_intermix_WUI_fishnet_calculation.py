...
# 步骤1：定义输入输出路径
gdp_raster_path = r"E:\data-hl\Origin data\rast_gdpTot_1990_2020_30arcsec.tif"
...
for wui_year in sorted(year_mapping.keys()):
    ...
    for gdp_year in sorted(gdp_years):
        ...
        # 执行分区统计并连接结果
        arcpy.sa.ZonalStatisticsAsTable(fishnet_for_analysis, zone_field, clipped_gdp_raster, temp_zonal_table, "DATA",
                                        "SUM")
