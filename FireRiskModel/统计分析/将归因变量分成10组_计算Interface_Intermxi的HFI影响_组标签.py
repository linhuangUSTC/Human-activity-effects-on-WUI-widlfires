...
r"""
程序名称:
    将多种归因参数分成 10 组，并将 Fire frequency 与 Normalized burned area 合并为 1x2 柱状图（组标签版）
...
功能:
    ...
    6. 对每个参数输出一张 1x2 图：左侧 Fire frequency，右侧 Normalized burned area。
...
输出:
    - B:\WUI\Picture\Grouped_Human Footprint_effect_by_{parameter}_interface_intermix_2003_2022_10groups_1x2_group_labels_barplot.png
"""
...
GROUP_RANGE_OUTPUT_PATH = OUTPUT_DIR / (
    f"Grouped_Human activity_effect_group_ranges_interface_intermix_{YEAR_TAG}_{GROUP_TAG}.xlsx"
)
CHUNK_SIZE = 250_000
...
def draw_metric_interface_intermix_bar_chart(
    ...
    if str(dataset["metric_slug"]) == "Fire_frequency":
        y_label = "Human activity effects\non fire frequency"
    else:
...
def plot_parameter_metrics_1x2(
    ...
    fig.subplots_adjust(bottom=0.34, top=0.95, left=0.075, right=0.99, wspace=0.28)
    output_path = OUTPUT_DIR / (
        f"Grouped_Human Footprint_effect_by_{parameter_config['slug']}_interface_intermix_{YEAR_TAG}_{GROUP_TAG}_1x2_group_labels_barplot.png"
    )
...
def main() -> None:
    ...
    for parameter_config in PARAMETER_CONFIGS:
        ...
        if len(metric_payloads) != len(DATASETS):
            continue
        for dataset in DATASETS:
            payload = metric_payloads[str(dataset["metric_slug"])]
            group_range_tables.append(
                build_group_range_table(
                    parameter_config=parameter_config,
                    dataset=dataset,
                    group_ranges=payload["group_ranges"],
                )
            )
