...
# -*- coding: utf-8 -*-
r"""
程序名称: 覆盖2003-2022年HFI影响四象限结果的气候带与land cover面积加权比例统计图

功能简介:
    1. 复用「Spatial_distribution_HFI的影响_双变量分布图_四象限.py」的四象限定义:
    ...
    3. 根据年份映射对应的 WUI 时段标签表:
       ...
       - 2018-2022 -> 2020
    4. 分别统计:
       - 各气候带(A/B/C/D)中的四象限面积加权比例
       - 各 land cover(CRP/FST/GRS/SHR/WET)中的四象限面积加权比例
    5. 输出 1x2 的 100% 堆叠柱状图:
       - All
       - Interface
       - Intermix
       不再输出 CSV 汇总表。
...
输出:
    - B:\WUI\Picture\Bivariate_HFI_quadrant_proportion_by_climate_landcover_all_years.png
    - B:\WUI\Picture\Bivariate_HFI_quadrant_all_years_Interface_Intermix_2x2.png
"""
...
KOPPEN_STATS_TEMPLATE = r"I:\Processing data\Koppe match\Grid_Clip_{wui_year}_Koppen_Stats.csv"
LANDCOVER_STATS_TEMPLATE = r"I:\Processing data\landcover match\Grid_Clip_{wui_year}_WUI_LandCover_Stats.csv"
...
OUTPUT_FIGURE = OUTPUT_DIR / "Bivariate_HFI_quadrant_proportion_by_climate_landcover_all_years.png"
OUTPUT_FIGURE_INTERFACE_INTERMIX = OUTPUT_DIR / "Bivariate_HFI_quadrant_all_years_Interface_Intermix_2x2.png"
OUTPUT_TABLE = OUTPUT_DIR / "Bivariate_HFI_quadrant_proportion_by_climate_landcover_all_years.xlsx"
BAR_ALPHA = 0.50
...
OTHER_LANDCOVER_TYPES = {"TUD", "IMP", "BAL", "PSI", "NaN", "WTR"}

CODE_TO_LANDCOVER: dict[int, str] = {}
for _landcover_name, _codes in LANDCOVER_CODE_GROUPS.items():
    for _code in _codes:
        CODE_TO_LANDCOVER[_code] = _landcover_name


def darken_color(color: str, factor: float = 0.66) -> tuple[float, float, float]:
    r, g, b = to_rgb(color)
...
def load_koppen_labels() -> pd.DataFrame:
    parts: list[pd.DataFrame] = []
    for wui_year in WUI_YEARS:
        csv_path = KOPPEN_STATS_TEMPLATE.format(wui_year=wui_year)
        df = pd.read_csv(csv_path, usecols=["GRID_ID", "Koppen_Category"])
        df["GRID_ID"] = pd.to_numeric(df["GRID_ID"], errors="coerce")
        df = df.dropna(subset=["GRID_ID"]).copy()
        df["GRID_ID"] = df["GRID_ID"].astype(int)
        df["Koppen_Category"] = df["Koppen_Category"].astype(str).str.strip().str.upper()
        df["WUI_Year"] = wui_year
        parts.append(df[["WUI_Year", "GRID_ID", "Koppen_Category"]])
    combined = pd.concat(parts, ignore_index=True)
    combined = combined.drop_duplicates(subset=["WUI_Year", "GRID_ID"], keep="first")
    return combined


def classify_landcover(top1_value: object, top2_value: object) -> str:
    top1 = pd.to_numeric(pd.Series([top1_value]), errors="coerce").iloc[0]
    top2 = pd.to_numeric(pd.Series([top2_value]), errors="coerce").iloc[0]

    if pd.isna(top1):
        return "Other"

    landcover = CODE_TO_LANDCOVER.get(int(top1), "Other")
    if landcover == "WTR" and not pd.isna(top2):
        landcover = CODE_TO_LANDCOVER.get(int(top2), "Other")

    if landcover in OTHER_LANDCOVER_TYPES or landcover not in LANDCOVER_ORDER:
        return "Other"
    return landcover


def load_landcover_labels() -> pd.DataFrame:
    parts: list[pd.DataFrame] = []
    for wui_year in WUI_YEARS:
        csv_path = LANDCOVER_STATS_TEMPLATE.format(wui_year=wui_year)
        df = pd.read_csv(csv_path, usecols=["GRID_ID", "Top1_Value", "Top2_Value"])
        df["GRID_ID"] = pd.to_numeric(df["GRID_ID"], errors="coerce")
        df = df.dropna(subset=["GRID_ID"]).copy()
        df["GRID_ID"] = df["GRID_ID"].astype(int)
        df["landcover"] = [
            classify_landcover(top1, top2)
            for top1, top2 in zip(df["Top1_Value"], df["Top2_Value"])
        ]
        df["WUI_Year"] = wui_year
        parts.append(df[["WUI_Year", "GRID_ID", "landcover"]])
    combined = pd.concat(parts, ignore_index=True)
    combined = combined.drop_duplicates(subset=["WUI_Year", "GRID_ID"], keep="first")
    return combined
...
def load_wui_attributes() -> pd.DataFrame:
    ...
    for wui_year in WUI_YEARS:
        ...
        df = pd.read_csv(csv_path, usecols=["GRID_ID", "main_WUI", "All_WUI_area"])
        df["GRID_ID"] = pd.to_numeric(df["GRID_ID"], errors="coerce")
        df = df.dropna(subset=["GRID_ID"]).copy()
        df["GRID_ID"] = df["GRID_ID"].astype(int)
        ...
        df["WUI_Year"] = wui_year
        parts.append(df[["WUI_Year", "GRID_ID", "wui_type", "wui_area"]])
    combined = pd.concat(parts, ignore_index=True)
    combined = combined.drop_duplicates(subset=["WUI_Year", "GRID_ID"], keep="first")
    return combined
...
def prepare_scope_plot_data(scope_df: pd.DataFrame, scope_name: str) -> tuple[pd.DataFrame, dict[str, int], pd.DataFrame, dict[str, int]] | None:
    if scope_df["GRID_ID"].nunique() == 0:
        print(f"All GRID_ID data are missing for {scope_name} in years [{FIRE_YEAR_START}, {FIRE_YEAR_END}].")
        return None

    climate_main = scope_df.loc[scope_df["Koppen_Category"].isin(CLIMATE_ORDER)].copy()
    landcover_main = scope_df.loc[scope_df["landcover"].isin(LANDCOVER_ORDER)].copy()

    if climate_main.empty:
        print(
            f"All GRID_ID data are missing for climate-zone statistics in {scope_name} "
            f"({FIRE_YEAR_START}-{FIRE_YEAR_END})."
        )
        return None
    if landcover_main.empty:
        print(
            f"All GRID_ID data are missing for land-cover statistics in {scope_name} "
            f"({FIRE_YEAR_START}-{FIRE_YEAR_END})."
        )
        return None

    omitted_climate = len(scope_df) - len(climate_main)
    omitted_landcover = len(scope_df) - len(landcover_main)
    print(
        f"{scope_name}: omitted from climate-zone bars (non A-D or missing): {omitted_climate}; "
        f"omitted from land-cover bars (Other or missing): {omitted_landcover}"
    )

    climate_summary = summarize_quadrant_proportions(
        climate_main,
        group_col="Koppen_Category",
        group_order=CLIMATE_ORDER,
    )
    landcover_summary = summarize_quadrant_proportions(
        landcover_main,
        group_col="landcover",
        group_order=LANDCOVER_ORDER,
    )

    climate_plot, climate_totals = build_plot_table(
        climate_summary,
        group_col="Koppen_Category",
        group_order=CLIMATE_ORDER,
    )
    landcover_plot, landcover_totals = build_plot_table(
        landcover_summary,
        group_col="landcover",
        group_order=LANDCOVER_ORDER,
    )
    return climate_plot, climate_totals, landcover_plot, landcover_totals
...
def plot_stacked_percentage_bars(
    ...
    *,
    ...
    bar_width = BAR_WIDTH * len(plot_table.index) / REFERENCE_CATEGORY_COUNT

    for quadrant in QUADRANT_ORDER:
        values = plot_table[quadrant].to_numpy(dtype=float)
        ax.bar(
            x,
            values,
            bottom=bottom,
    ...
    for xpos, group in enumerate(plot_table.index):
        if totals.get(group, 0) == 0:
            ax.text(xpos, 50, "NA", ha="center", va="center", fontsize=13, color="black")
...
def merge_quadrant_labels(quadrant_df: pd.DataFrame) -> pd.DataFrame:
    koppen_df = load_koppen_labels()
    landcover_df = load_landcover_labels()
    wui_attribute_df = load_wui_attributes()

    merged = quadrant_df.merge(koppen_df, on=["WUI_Year", "GRID_ID"], how="left")
    merged = merged.merge(landcover_df, on=["WUI_Year", "GRID_ID"], how="left")
    merged = merged.merge(wui_attribute_df, on=["WUI_Year", "GRID_ID"], how="left")
    merged["Koppen_Category"] = merged["Koppen_Category"].fillna("Other")
    merged["landcover"] = merged["landcover"].fillna("Other")
...
def build_output_table(
    ...
    *,
    ...
    table = table.reset_index().rename(columns={table.index.name or table.columns[0]: "group_code"})
    if grouping_type == "Climate":
        label_map = CLIMATE_LABELS
    else:
        label_map = LANDCOVER_LABELS
    table["group_label"] = table["group_code"].map(label_map).fillna(table["group_code"])
    table["scope"] = scope_name
    table["grouping_type"] = grouping_type
...
def write_output_tables(merged: pd.DataFrame, output_table: Path) -> None:
    ...
    with pd.ExcelWriter(output_table, engine="openpyxl") as writer:
        for scope_name, scope_df in scope_frames.items():
            prepared = prepare_scope_plot_data(scope_df, scope_name)
            if prepared is None:
                continue
            climate_plot, climate_totals, landcover_plot, landcover_totals = prepared
            climate_table = build_output_table(
                climate_plot,
                climate_totals,
                scope_name=scope_name,
                grouping_type="Climate",
            )
            landcover_table = build_output_table(
                landcover_plot,
                landcover_totals,
                scope_name=scope_name,
                grouping_type="Land cover",
            )
            climate_table.to_excel(writer, sheet_name=f"{scope_name}_climate", index=False)
            landcover_table.to_excel(writer, sheet_name=f"{scope_name}_landcover", index=False)


def render_scope_figure(
    ...
    *,
    ...
    fig, axes = plt.subplots(1, 2, figsize=(14.8, 5.44), sharey=True)

    plot_stacked_percentage_bars(
        axes[0],
        climate_plot,
        climate_totals,
        x_labels=[CLIMATE_LABELS[item] for item in CLIMATE_ORDER],
    ...
    plot_stacked_percentage_bars(
        ...
        landcover_plot,
        landcover_totals,
        x_labels=[LANDCOVER_LABELS[item] for item in LANDCOVER_ORDER],
        y_label="",
        xtick_fontsize=XTICK_LABEL_FONT_SIZE,
    )
    fig.subplots_adjust(left=0.08, right=0.99, top=0.94, bottom=0.12, wspace=0.12)
...
def render_interface_intermix_combined(interface_df: pd.DataFrame, intermix_df: pd.DataFrame, output_figure: Path) -> bool:
    interface_prepared = prepare_scope_plot_data(interface_df, "Interface")
    intermix_prepared = prepare_scope_plot_data(intermix_df, "Intermix")
    if interface_prepared is None or intermix_prepared is None:
        return False

    interface_climate_plot, interface_climate_totals, interface_landcover_plot, interface_landcover_totals = interface_prepared
    intermix_climate_plot, intermix_climate_totals, intermix_landcover_plot, intermix_landcover_totals = intermix_prepared

    fig, axes = plt.subplots(1, 2, figsize=(14.8, 5.44), sharey=True)

    plot_grouped_stacked_percentage_bars(
        axes[0],
        interface_climate_plot,
        interface_climate_totals,
        intermix_climate_plot,
        intermix_climate_totals,
        x_labels=[CLIMATE_LABELS[item] for item in CLIMATE_ORDER],
        y_label="WUI area proportion",
        xtick_fontsize=XTICK_LABEL_FONT_SIZE,
    )
    ...
    plot_grouped_stacked_percentage_bars(
        axes[1],
        interface_landcover_plot,
        interface_landcover_totals,
        intermix_landcover_plot,
        intermix_landcover_totals,
        x_labels=[LANDCOVER_LABELS[item] for item in LANDCOVER_ORDER],
        y_label="",
        xtick_fontsize=XTICK_LABEL_FONT_SIZE,
    )
    ...
    fig.subplots_adjust(left=0.08, right=0.99, top=0.89, bottom=0.16, wspace=0.12)
    fig.savefig(str(output_figure), dpi=1200, bbox_inches="tight")
    plt.close(fig)
    print(f"Saved figure to: {output_figure}")
    return True
