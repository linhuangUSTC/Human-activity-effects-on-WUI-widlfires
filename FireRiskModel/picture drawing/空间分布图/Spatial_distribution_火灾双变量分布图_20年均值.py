...
# -*- coding: utf-8 -*-
r"""
程序名称: Fire frequency 与 Normalized burned area 双变量空间分布图（2003-2022 均值）

功能简介:
    1. 从两个分类建模 CSV 目录中分别读取 Fire_frequency 与 Normalized_fire_area。
    2. 仅保留 GRID_ID、Year 和对应指标列，并在全局范围内合并。
    3. 根据 Year 选择对应的 WUI 渔网图层，并合并 2005/2010/2015/2020 四套图层:
       - 2003-2007 -> Grid_Clip_2005_WUI
       - 2008-2012 -> Grid_Clip_2010_WUI
       - 2013-2017 -> Grid_Clip_2015_WUI
       - 2018-2022 -> Grid_Clip_2020_WUI
    4. 对每个 GRID_ID，基于 2003-2022 全部可用年份计算 Fire frequency 和
       Normalized burned area 的均值；若某格网不足 20 年，则仅对其可用年份求均值。
    5. 将两个多年均值分别分成 3 个等级，再组合成 3x3 双变量颜色矩阵绘制全球空间分布图。
    6. 底图、渔网和版式尽量保持与原脚本一致。
...
输出:
    - B:\WUI\Picture\Bivariate_FireFrequency_NormalizedFireArea_Mean_2003_2022.png
"""
...
OUTPUT_FILENAME = "Bivariate_FireFrequency_NormalizedFireArea_Mean_2003_2022.png"
OUTPUT_DPI = 1200
...
def main() -> None:
    ...
    for wui_year in WUI_YEARS:
        ...
        print(f"Loaded WUI layer {layer}: {len(fishnet_base)} features")
    ...
    print("\n=== Processing 2003-2022 mean ===")
    mean_metric = (
        metric_all.groupby("GRID_ID", as_index=False)
        .agg(
            Fire_frequency=("Fire_frequency", "mean"),
            Normalized_fire_area=("Normalized_fire_area", "mean"),
            available_years=("Year", "nunique"),
        )
    )

    mean_metric["Fire_frequency_class"] = classify_three_classes(mean_metric["Fire_frequency"])
    mean_metric["Normalized_fire_area_class"] = classify_three_classes(mean_metric["Normalized_fire_area"])
    fire_frequency_labels, fire_frequency_scale_text = get_class_representative_values(
        mean_metric["Fire_frequency"],
        mean_metric["Fire_frequency_class"],
    )
    normalized_fire_area_labels, normalized_fire_area_scale_text = get_class_representative_values(
        mean_metric["Normalized_fire_area"],
        mean_metric["Normalized_fire_area_class"],
    )

    valid_class = (
        mean_metric["Fire_frequency_class"].notna() &
        mean_metric["Normalized_fire_area_class"].notna()
    )
    mean_metric["bivariate_key"] = pd.Series([pd.NA] * len(mean_metric), dtype="object")
    mean_metric.loc[valid_class, "bivariate_key"] = pd.Series(
        list(
            zip(
                mean_metric.loc[valid_class, "Fire_frequency_class"].astype(int),
                mean_metric.loc[valid_class, "Normalized_fire_area_class"].astype(int),
            )
        ),
        index=mean_metric.index[valid_class],
        dtype="object",
    )

    mean_ids = set(mean_metric["GRID_ID"].astype(int).tolist())
    fishnet_mean = fishnet_all[fishnet_all["GRID_ID"].isin(mean_ids)].copy()
    fishnet_mean = fishnet_mean.merge(mean_metric, on="GRID_ID", how="left")

    output_path = os.path.join(OUTPUT_DIR, OUTPUT_FILENAME)
    plot_bivariate_map(
        continent=continent,
        fishnet=fishnet_mean,
        map_label="2003-2022 mean",
