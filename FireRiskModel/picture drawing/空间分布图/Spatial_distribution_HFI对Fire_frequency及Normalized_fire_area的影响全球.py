...
# -*- coding: utf-8 -*-
r"""
程序名称: Human Footprint对Fire frequency及Normalized burned area的影响全球分布图
功能简介:
    1) 分别读取 Fire frequency 与 Normalized burned area 的 HFI_effect_*.csv
    2) 仅保留 GRID_ID、Year、(delta/diff)_contribution、delta_sig（Year=2003-2022），每年绘制一张图
    3) 根据 Year 选择 WUI 渔网图层：
       - 2003-2007 -> Grid_Clip_2005_WUI
       - 2008-2012 -> Grid_Clip_2010_WUI
       - 2013-2017 -> Grid_Clip_2015_WUI
       - 2018-2022 -> Grid_Clip_2020_WUI
    4) 按 GRID_ID 匹配并用 colormap 绘制显著格网的 delta_contribution
    5) 不显著格网（delta_sig == FALSE）用灰色表示
    6) 以 I:/Data_huanglin/map/world continent boundary/continent.shp 为底图（WGS-84）
    7) 输出图片到 B:/WUI/Picture
"""
...
DATASETS = [
    {
        ...
        "output_filename_template": "Human Footprint对Fire_frequency的影响全球分布图_{year}.jpg",
        "colorbar_label": "Human Footprint effect on fire frequency",
    },
    {
        "metric_title": "Normalized burned area",
        "effect_source": r"B:\WUI\Picture and statistic results\HFI的净效应-固定效应结果\csv\Fire area",
        "output_filename_template": "Human Footprint对Normalized_burn_area的影响全球分布图_{year}.jpg",
        "colorbar_label": "Human Footprint effect on normalized burned area",
    },
]
...
def _coerce_bool_series(series: pd.Series) -> pd.Series:
    ...
    # R 写出的 TRUE/FALSE 默认会被 pandas 读成 object
...
def load_effect_data(effect_source: str, year_start: int, year_end: int) -> pd.DataFrame:
    ...
    # 若出现重复 (Year, GRID_ID)（例如多个输入文件包含同一 GRID_ID），做聚合处理
    combined = (
...
def read_wui_fishnet_filtered(
    ...
    while True:
        try:
            ...
            # OpenFileGDB/pyogrio 在 `IN (...)` 太长/元素太多时会报 Invalid SQL query
            msg = str(e)
...
def plot_fire_year_map(
    *,
    ...
    if len(fishnet_data) > 0:
        ...
        print("No delta_contribution values found for this year.")
    ...
    for dataset in DATASETS:
        ...
        for wui_year in sorted(years_by_wui_year):
            ...
            for fire_year in year_group:
                ...
                effect_year = effect_all[effect_all["Year"] == fire_year][["GRID_ID", "delta_contribution", "delta_sig"]]
