...
# ------------------------------------------------------------------------
# 程序: GLMM_for_normalized_fire_area_零膨胀OrdBeta_自然样条.R
# 功能: 使用零膨胀+Ordbeta分布的广义线性混合模型(GLMM)对归一化火灾面积进行建模分析，具体功能包括：
#       - 直接使用Normalized fire area (Fire area / WUI area) 作为响应变量
#       - 仅使用HFI（人类足迹指数）作为自变量，采用自然样条进行非线性建模
#       - 计算模型评估指标：边际R²、条件R²、MSE、RMSE、MAE（基于归一化火灾面积）
#       - 批量处理多份CSV数据文件
# 输入: CSV文件(路径: I:/Processing data/渔网分类建模/Fire_area_with_WUI_area_offset_conclude_0_分类建模数据_Basic/)
#       - 必须包含列：Normalized_fire_area、HFI、GRID_ID
# 输出: 模型报告(TXT)、边际效应图(PNG)与模型RDS，输出路径: B:/WUI/Picture/
...
# 检查模型收敛性
# 与 GLMM_for_fire_num_NB_自然样a条_均值图.R 保持一致
# ------------------------------------------------------------------------
# 检查模型收敛性
# ------------------------------------------------------------------------
...
generate_model_report <- function(model, df, file_name, output_dir, y_col, x_col, offset_col, group_var, y_pred, pred_freq, obs_freq, r_squared, convergence_result) {
  report_file <- file.path(output_dir, paste0("GLMM_NaturalSpline_ZIOrdbeta_HFI_only_", file_name, "_report.txt"))
  ...
  cat(paste0("\n", strrep("=", 70), "\n"))
  cat(paste0("Natural Spline Zero-inflated Ordbeta-GLMM (logit link) Normalized Fire Area Model Report with HFI Only - ", file_name, "\n"))
...
generate_marginal_effect_plot <- function(model, df, file_name, output_dir, x_col, y_col, offset_col, group_var, standardized_stats) {
  extract_classification_id <- function(x) {
  ...
  classification_id <- extract_classification_id(file_name)
  ...
  pred_fit <- pred_result$fit
  pred_se <- pred_result$se.fit
  ci_lower <- pred_fit - 1.96 * pred_se
  ci_upper <- pred_fit + 1.96 * pred_se
  ...
  for(i in 1:n_bins) {
    ...
    if(nrow(bin_data) > 0) {
      ...
      freq_mean <- mean(bin_data$Normalized_fire_area, na.rm = TRUE)  # 使用均值
      freq_se <- sd(bin_data$Normalized_fire_area, na.rm = TRUE) / sqrt(length(bin_data$Normalized_fire_area))  # 计算标准误
      freq_count <- length(bin_data$Normalized_fire_area)
  ...
  marginal_plot <- ggplot(pred_data, aes(x = hfi_original)) +
    ...
    theme(
      ...
      plot.title = element_text(family = "Arial", hjust = 0.5, size = rel(1.6))
    )
  
  # 保存边际效应图
  marginal_file <- file.path(output_dir, paste0("GLMM_NaturalSpline_ZIOrdbeta_HFI_only_", file_name, "_marginal_effect.png"))
...
# 保存模型与可复用信息
save_model_artifact <- function(model, file_name, output_dir, standardized_stats, spline_info, formula_str, zi_formula_str, x_col, y_col, offset_col, group_var) {
  model_file <- file.path(output_dir, paste0("GLMM_NaturalSpline_ZIOrdbeta_HFI_only_", file_name, "_model.rds"))
  saveRDS(list(
    model = model,
    standardized_stats = standardized_stats,
    spline_info = spline_info,
    formula = formula_str,
    zi_formula = zi_formula_str,
    x_col = x_col,
    y_col = y_col,
    offset_col = offset_col,
    group_var = group_var,
    model_type = "ZI_Ordbeta_natural_spline",
...
process_single_csv <- function(csv_path, output_dir) {
  tryCatch({
    ...
    use_cols <- c(y_col, original_x_col, group_var)
    df <- df[, use_cols]
    df <- na.omit(df)
    ...
    formula_str <- paste(y_col, "~", spline_terms, "+ (1|", group_var, ")")
    
    # 拟合零膨胀+Ordbeta-GLMM模型（使用自然样条）
    zi_formula_str <- "~1"
    final_model <- tryCatch({
      ...
      cat(paste0("零膨胀+Ordbeta GLMM模型拟合失败: ", e$message, "，跳过此文件\n"))
      stop(paste0("零膨胀+Ordbeta GLMM模型拟合失败: ", e$message))
    })
    ...
    # 保存模型与可复用信息
    save_model_artifact(final_model, file_name, output_dir, standardized_stats, spline_info, formula_str, zi_formula_str, standardized_x_col, y_col, offset_col, group_var)
...
# 主函数
main <- function() {
  input_dir <- "E:/data-hl/Origin data/渔网分类建模/Fire_area_with_WUI_area_offset_conclude_0_分类建模数据_Basic"
  output_dir <- "E:/data-hl/Picture"
