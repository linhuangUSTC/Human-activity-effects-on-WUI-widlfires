...
# ------------------------------------------------------------------------
# 程序: GLMM_for_normalized_fire_area_自然样条_simplify.R
# 功能: 使用Ordbeta分布的广义线性混合模型(GLMM)对归一化火灾面积进行建模分析，具体功能包括：
...
# 检查模型收敛性
# 与 GLMM_for_fire_num_NB_自然样条_均值图.R 保持一致
# ------------------------------------------------------------------------
# 检查模型收敛性
# ------------------------------------------------------------------------
...
generate_model_report <- function(model, df, file_name, output_dir, y_col, x_col, offset_col, group_var, y_pred, pred_freq, obs_freq, r_squared, convergence_result) {
  report_file <- file.path(output_dir, paste0("GLMM_NaturalSpline_Ordbeta_HFI_only_", file_name, "_report.txt"))
  ...
  cat(paste0("\n", strrep("=", 70), "\n"))
  cat(paste0("Natural Spline Ordbeta-GLMM (logit link) Normalized Fire Area Model Report with HFI Only - ", file_name, "\n"))
...
generate_marginal_effect_plot <- function(model, df, file_name, output_dir, x_col, y_col, offset_col, group_var, standardized_stats) {
  ...
  # 保存边际效应图
  marginal_file <- file.path(output_dir, paste0("GLMM_NaturalSpline_Ordbeta_HFI_only_", file_name, "_marginal_effect.png"))
...
save_model_artifact <- function(model, file_name, output_dir, standardized_stats, spline_info, formula_str, x_col, y_col, offset_col, group_var) {
  model_file <- file.path(output_dir, paste0("GLMM_NaturalSpline_Ordbeta_HFI_only_", file_name, "_model.rds"))
  saveRDS(list(
    model = model,
    ...
    group_var = group_var,
    model_type = "Ordbeta_natural_spline",
...
process_single_csv <- function(csv_path, output_dir) {
  tryCatch({
    ...
    final_model <- tryCatch({
      ...
      cat(paste0("Ordbeta GLMM模型拟合失败: ", e$message, "，跳过此文件\n"))
      stop(paste0("Ordbeta GLMM模型拟合失败: ", e$message))
    })
