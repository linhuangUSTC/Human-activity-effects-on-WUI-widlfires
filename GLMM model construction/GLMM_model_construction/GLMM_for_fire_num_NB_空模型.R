...
# ------------------------------------------------------------------------
# 程序: GLMM_for_fire_num_NB_noHFI.R
# 功能: 使用负二项分布的广义线性混合模型(GLMM)对火灾数量数据建模，
#       仅包含offset与随机效应，不包含HFI等自变量；仅保存模型RDS。
# 输入: CSV文件(路径: I:/Processing data/渔网分类建模/Fire_num_with_WUI_area_offset_conclude_0_分类建模数据_Basic/)
#       - 必须包含列：All_fire_num、All_WUI_area、GRID_ID
# 输出: 模型RDS，输出路径: B:/WUI/Picture/
# ------------------------------------------------------------------------
...
generate_model_report <- function(model, file_name, output_dir, pred_freq, obs_freq, r_squared, convergence_result) {
  report_file <- file.path(output_dir, paste0("GLMM_Null_NB_without_HFI_", file_name, "_report.txt"))
  ...
  cat(paste0("\n", strrep("=", 70), "\n"))
  cat(paste0("Null NB-GLMM Fire Frequency Model Report without HFI - ", file_name, "\n"))
...
save_model_artifact <- function(model, file_name, output_dir, formula_str, y_col, offset_col, group_var) {
  model_file <- file.path(output_dir, paste0("GLMM_NB_noHFI_", file_name, "_model.rds"))
  saveRDS(list(
    model = model,
    formula = formula_str,
    y_col = y_col,
...
process_single_csv <- function(csv_path, output_dir) {
  tryCatch({
    ...
    required_cols <- c("All_fire_num", "All_WUI_area", "GRID_ID")
    ...
    use_cols <- c(y_col, offset_col, group_var)
    df <- df[, use_cols]
    df <- na.omit(df)
    
    df <- df[is.finite(df[[y_col]]) & is.finite(df[[offset_col]]), ]
    df <- df[df[[offset_col]] > 0 & df[[y_col]] >= 0, ]
    if (nrow(df) < 5) {
      cat(paste0("有效样本过少（去除缺失/无效值后 <5）: ", file_name, "\n"))
      return(FALSE)
    }
    
    df[[group_var]] <- as.factor(df[[group_var]])
    
    formula_str <- paste(y_col, "~ 1 + offset(log(", offset_col, ")) + (1|", group_var, ")")
    ...
    final_model <- tryCatch({
      ...
      cat(paste0("NB GLMM模型拟合失败: ", e$message, "，跳过此文件\n"))
