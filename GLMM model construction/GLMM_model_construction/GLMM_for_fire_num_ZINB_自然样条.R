...
# ------------------------------------------------------------------------
# 程序: GLMM_with_HFI_only_for_fire_num_with_WUI_area_offset_NS_ZINB.R
# 功能: 使用零膨胀负二项分布的广义线性混合模型(GLMM)对火灾数量数据进行建模分析，具体功能包括：
...
# 输出: 模型报告(TXT)、边际效应图(PNG)和模型对象(RDS)，输出路径: B:/WUI/Picture/
#       - TXT报告：包含模型参数估计、拟合优度指标
#       - PNG图片：HFI 对 Fire frequency 的边际效应曲线
#       - RDS文件：保存 ZINB 自然样条模型及复用所需信息
# ------------------------------------------------------------------------
...
generate_model_report <- function(model, df, file_name, output_dir, y_col, x_col, offset_col, group_var, y_pred, pred_freq, obs_freq, r_squared, convergence_result) {
  ...
  cat(paste0("\n", strrep("=", 70), "\n"))
  cat(paste0("Natural Spline ZINB-GLMM (log link) Fire Frequency Model Report with HFI Only - ", file_name, "\n"))
...
save_model_artifact <- function(model, file_name, output_dir, standardized_stats, spline_info, formula_str, zi_formula_str, x_col, y_col, offset_col, group_var) {
  ...
  saveRDS(list(
    ...
    group_var = group_var,
    model_type = "ZINB_natural_spline",
    response_transform = "count_per_area: Fire_frequency"
  ), model_file)
...
process_single_csv <- function(csv_path, output_dir) {
  tryCatch({
    ...
    final_model <- tryCatch({
      ...
      cat(paste0("ZINB GLMM模型拟合失败: ", e$message, "，跳过此文件\n"))
      stop(paste0("ZINB GLMM模型拟合失败: ", e$message))
    })
