...
# ------------------------------------------------------------------------
# 程序: 量化HFI对归一化火灾面积（Normalized fire area）的影响
# 功能:
#   - 读取 B:/WUI/Picture and statistic results/model construction/GLMM Ordbeta 自然样条 Normalized fire area 下的模型产物 (*.rds)
#   - 读取输入目录下的 CSV 数据
...
delta_method_effects <- function(beta, V, X, X0) {
  eta <- as.vector(X %*% beta)
  eta0 <- as.vector(X0 %*% beta)
  mu_resp <- plogis(eta)
  mu0_resp <- plogis(eta0)
  delta <- mu_resp - mu0_resp
  mu_eps <- pmax(mu_resp, .Machine$double.eps)
  mu0_eps <- pmax(mu0_resp, .Machine$double.eps)
  logR <- log(mu_eps) - log(mu0_eps)
  R <- mu_eps / mu0_eps
  
  se_delta <- rep(NA_real_, length(mu_resp))
  se_logR <- rep(NA_real_, length(mu_resp))
  if (!is.null(V)) {
    X_mu <- sweep(X, 1, mu_resp * (1 - mu_resp), "*")
    X0_mu0 <- sweep(X0, 1, mu0_resp * (1 - mu0_resp), "*")
    gprime <- X_mu - X0_mu0
    var_delta <- rowSums((gprime %*% V) * gprime)
    se_delta <- sqrt(pmax(var_delta, 0))
    
    X_logmu <- sweep(X, 1, 1 - mu_resp, "*")
    X0_logmu0 <- sweep(X0, 1, 1 - mu0_resp, "*")
    diffX <- X_logmu - X0_logmu0
...
process_single_csv <- function(csv_path, rds_dir, output_dir) {
  ...
  rds_path <- file.path(rds_dir, paste0("GLMM_NaturalSpline_Ordbeta_HFI_only_", file_name, "_model.rds"))
  ...
  y_col <- if (!is.null(model_artifact$y_col)) model_artifact$y_col else "Normalized_fire_area"
  offset_col <- if (!is.null(model_artifact$offset_col)) model_artifact$offset_col else NULL
  ...
  wui_area <- if ("All_WUI_area" %in% colnames(df)) df[["All_WUI_area"]] else rep(NA_real_, nrow(df))
  
  valid <- is.finite(hfi_raw)
  ...
  if (any(valid)) {
    ...
    names(newdata) <- c(x_col, group_var)
    ...
    newdata_base <- add_spline_columns(newdata_base[, c(x_col, group_var)], hfi_std_base, spline_info)
    ...
    dm <- delta_method_effects(beta, V, X, X0)
    
    pred_freq[valid] <- dm$mu_resp
    base_freq[valid] <- dm$mu0_resp
  ...
  out_df <- data.frame(
    ...
    pred_normalized_fire_area = pred_freq,
    baseline_normalized_fire_area = base_freq,
