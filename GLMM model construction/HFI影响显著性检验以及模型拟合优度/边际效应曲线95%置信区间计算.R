...
# 输出: 边际效应图（PNG）
#       - B:/WUI/Picture/*_marginal_effect.png
# ------------------------------------------------------------------------
...
make_plot <- function(pred_df, y_label, out_path) {
  ...
  sci_label_1dec <- function(x) {
    ifelse(is.finite(x), sprintf("%.1e", x), NA_character_)
  }
  p <- ggplot(pred_df, aes(x = hfi_original)) +
    ...
    scale_y_continuous(labels = sci_label_1dec) +
    ggtitle(pred_df$class_id[1]) +
    theme_bw() +
    theme(
...
process_rds <- function(rds_path, output_dir) {
  ...
  if (!is.na(y_col) && grepl("Fire_num", y_col, ignore.case = TRUE)) {
    y_label <- expression(Fire~frequency~(per~km^2))
  } else if (!is.na(y_col) && grepl("Fire_area", y_col, ignore.case = TRUE)) {
    y_label <- "Normalized Fire Area"
  ...
  out_path <- file.path(output_dir, paste0(base, "_marginal_effect.png"))
