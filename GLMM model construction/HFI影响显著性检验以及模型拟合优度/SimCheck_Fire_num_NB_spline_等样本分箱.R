...
#       评估模型能否生成与真实 Fire frequency 数据相似的整体分布。
...
for (i in seq_along(rds_files)) {
  ...
  if (!is.null(sim_error_message)) {
    ...
    cleanup_vars <- intersect(
      c(
        "mf", "df_raw", "df_required", "df_fallback", "df_aligned", "y_obs", "obs_stats",
        "ppc_info", "sim_zero", "sim_mean", "sim_var", "sim_q95", "sim_q99", "sim_max", "sim_tail",
        "sim_bin_means_all", "model_bundle", "model_obj", "sim_error_message", "sim_chunks"
      ),
      ls()
    )
  ...
  cleanup_vars <- intersect(
    c(
      "mf", "df_raw", "df_required", "df_fallback", "df_aligned", "y_obs", "obs_stats",
      "ppc_info", "sim_zero", "sim_mean", "sim_var", "sim_q95", "sim_q99", "sim_max", "sim_tail",
      "sim_bin_means_all", "s_zero", "s_mean", "s_var", "s_q95", "s_q99", "s_max", "s_tail",
      "model_bundle", "model_obj", "sim_error_message", "sim_chunks", "hfi_plot", "row_idx"
    ),
    ls()
  )
  rm(list = cleanup_vars)
