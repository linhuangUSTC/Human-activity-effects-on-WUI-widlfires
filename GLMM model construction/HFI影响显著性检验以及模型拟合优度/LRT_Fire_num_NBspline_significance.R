...
# ------------------------------------------------------------------------
# 程序: LRT_Fire_num_NB_HFI_vs_noHFI.R
# 功能: 根据分类ID匹配两类模型RDS（NB+HFI自然样条 vs NB无HFI），进行LRT检验并输出CSV
#      LRT检验用于判断包含HFI的模型是否相对无HFI的空模型带来显著拟合改进。
# 输入:
#   - I:/Processing data/渔网分类建模/火灾分类数据汇总/分类建模火灾数据汇总统计_Basic.xlsx
#     sheet: Fire_num_over100, column: 分类ID
#   - B:/WUI/Picture and statistic results/model construction/Fire num NB without HFI (NB无HFI空模型RDS)
#   - B:/WUI/Picture and statistic results/model construction/GLMM NB 自然样条曲线 Fire num (NB+HFI自然样条模型RDS)
# 输出:
...
rds_dir_zinb <- "B:/WUI/Picture and statistic results/model construction/Fire num NB without HFI"
...
for (id in class_ids) {
  ...
  if (m_zi$note == "no_match") {
    ...
    note_vec <- c(note_vec, paste0("NoHFI_", m_zi$note))
  }
  ...
  if (!is.na(m_zi$path) && is.na(m_ns$path)) better_model <- "NoHFI"
    ...
    tryCatch({
      ...
      if (!inherits(model_zi, "glmmTMB") || !inherits(model_ns, "glmmTMB")) {
        ...
        if (is.finite(aic_zi) && is.finite(aic_ns) && is.finite(bic_zi) && is.finite(bic_ns)) {
          aic_better <- ifelse(aic_ns < aic_zi, "NB+Spline", "NoHFI")
          bic_better <- ifelse(bic_ns < bic_zi, "NB+Spline", "NoHFI")
          if (!is.na(hfi_sig) && hfi_sig == "Yes" && aic_better == "NB+Spline" && bic_better == "NB+Spline") {
            better_model <- "NB+Spline"
          } else if (!is.na(hfi_sig) && hfi_sig == "No" && aic_better == "NoHFI" && bic_better == "NoHFI") {
            better_model <- "NoHFI"
  ...
  results <- rbind(results, data.frame(
    分类ID = id,
    model_NoHFI = ifelse(is.na(m_zi$path), "", basename(m_zi$path)),
